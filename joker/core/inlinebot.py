...
def main_menu():
    text = f"**- [سـورس فايبرو](https://t.me/FFAARRSS1)\
        \n المسـاعد\
        \n\
        \n◽ فايبرو لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("l313l"):
            buttons = [
                (
                    ...
                    Button.url(" joker UsᴇʀBoᴛ", "https://t.me/FFAARRSS1"),
                )
            ]
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس فايبرو 🧸♥",
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس فايبرو 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس فايبرو 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/FFAARRSS1"),
                Button.url(
                    "كـروب المـساعدة 📬",
                    "https://t.me/FFAARRSS11",
                ),
            )
        ]
        markup = event.client.build_reply_markup(buttons)
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت فايبرو 🧸♥](https://t.me/FFAARRSS1)**\
            \n\
            \n❤ فايبرو هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [كروب المسـاعدة](https://t.me/FFAARRSS11)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
